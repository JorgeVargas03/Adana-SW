...
const instructorNavigation = [
  { name: 'MIS CLASES', to: '/instructor/home' },
  { name: 'INSTRUCTORES', to: '/instructors' },
  { name: 'BIENESTAR', to: '/wellness' },
];
...
export default function Navbar() {
  ...
  const handleSignOut = () => {
    toast.info('Sesión cerrada con éxito, hasta luego!');
    removeToken();
  ...
  const validateToken = () => {
    ...
    setHasToken(valid);
    if (!valid) setUserRole(null);
  };

  const getNavigationByRole = (role) => {
    switch (role) {
      case 'instructor': return instructorNavigation;
      case 'administrador': return adminNavigation;
      default: return baseNavigation;
    }
  };

  const getTitleByRole = (role) => {
    switch (role) {
      case 'instructor': return 'Adana Pilates Estudio - Instructor';
      case 'administrador': return 'Adana Pilates Estudio - Administrador';
      default: return 'Adana Pilates Estudio';
    }
  };
      ...
      {({ open }) => (
        <>
                    ...
                    className={({ isActive }) =>
                      classNames(
                        ...
                        (isTransparentRoute && !isScrolled)
                          ...
                          : isActive ? 'text-fontdef' : 'text-fontdef hover:text-fontdef/90',
                ...
                {hasToken ? (
                  <div className="relative ml-3" ref={dropdownRef}>
                    ...
                    >
                      ...
                      <img className="size-8 rounded-full cursor-pointer" src={profilePicture || logo} alt="User profile" />
                    </button>
                    ...
                    {dropdownOpen && (
                      <div
                        ...
                        tabIndex="-1"
                      >
                        <a onClick={toProfile} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">Tu Perfil</a>
                        <button onClick={handleSignOut} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">Cerrar sesión</button>
                      </div>
                    )}
                  </div>
                ) : (
          ...
          <Disclosure.Panel className="sm:hidden">
            <div className={`px-2 pb-3 pt-2 ${(isTransparentRoute && !isScrolled) ? 'bg-black/90' : 'bg-white'}`}>
              {navigation.map((item) => (
                <NavLink
                  key={item.name}
                  to={item.to}
